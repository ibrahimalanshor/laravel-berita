<section id="komentar" class="px-4 scroll-mt-20 sm:p-0">
    <div class="flex flex-col gap-2 mb-2 lg:flex-row lg:items-center lg:justify-between">
        ...
        @guest
            <x-home.base.button icon="icon-[tabler--brand-google-filled]" tag-name="a" color="bordered" class="w-full sm:w-auto" href="{{ route('auth.google') }}">
    ...
    @auth
        <x-home.comment.form type="create" :article="$article->slug" />
    @endauth
    ...
    <div class="space-y-4">
        ...
        @else
            <form class="flex flex-wrap gap-2">
                <x-home.base.button type="submit" name="sort" value="latest" color="{{ $sort === 'latest' ? 'primary' : 'bordered' }}" size="sm">Terbaru</x-base.button>
                <x-home.base.button type="submit" name="sort" value="oldest" color="{{ $sort === 'oldest' ? 'primary' : 'bordered' }}" size="sm">Terlama</x-base.button>
                <x-home.base.button type="submit" name="sort" value="popular" color="{{ $sort === 'popular' ? 'primary' : 'bordered' }}" size="sm">Terpopuler</x-base.button>
            </form>
            ...
            @if ($comments->count() < $total)
                <div class="flex items-center justify-center">
                    <x-home.base.button data-load-more="#comment-list" data-total="{{ $total }}" data-page="1" size="sm" class="text-sm" color="bordered" icon="icon-[tabler--reload]">Lebih banyak</x-base.button>
                </div>
            @endif
        @endif
    </div>
