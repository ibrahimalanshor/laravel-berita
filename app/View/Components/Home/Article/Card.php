...
class Card extends Component
{
    ...
    {
        if ($this->type === 'highlight' || $this->type === 'editor' || $this->type === 'related-article') {
            $this->thumbnailSet = collect($this->article->thumbnails['16x9'])
                ->map(function ($fileUrl, $width) {
                    return "$fileUrl {$width}w";
                })
                ->join(', ');
        } else if ($this->type === 'flash' || $this->type === 'article-category' || $this->type === 'bookmark' || $this->type === 'favorite') {
    ...
    private function getClassList(string $type) : array
    {
        ...
        if ($type === 'bookmark' || $type === 'favorite') {
            return [
                ...
                'title-normal' => '',
                'title-featured' => '',
                'thumbnail' => 'w-16 aspect-square object-cover',
                'thumbnail-normal' => '',
                'thumbnail-featured' => '',
                'thumbnail-link' => 'relative',
