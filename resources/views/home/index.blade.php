...
@section('content')
    <x-home.base.container :paddless="true" class="sm:px-4">
            ...
            <div class="splide__track">
                <div class="splide__list splide__list__grid sm:grid-cols-4 sm:grid-rows-2 sm:gap-6">
                    @foreach ($highlights as $article)
                        <x-home.article.card type="highlight" :featured="$loop->first" :article="$article" :slide-on-mobile="true" @class(['splide__slide', 'col-span-1 row-span-1 sm:col-span-2 sm:row-span-2' => $loop->first]) />
                    @endforeach
                </div>
            </div>
        </section>
    ...
    <div class="flex flex-col">
        <x-home.article.section class="lg:order-last">
            <x-home.base.container class="space-y-2 lg:space-y-4">
                <x-home.article.section-title :read-more-url="route('news')">Berita Terbaru</x-article.section-title>

                <div class="grid grid-cols-1 gap-4 lg:grid-cols-3 lg:gap-6">
                    @foreach ($flash as $article)
                        <x-home.article.card type="flash" :article="$article" />
                    @endforeach
                </div>
            </x-base.container>
        </x-article.section>
            ...
            <x-home.article.section>
                <x-home.base.container :paddless="true">
                    <div id="premium-article" class="splide space-y-4">
                        <div class="px-4">
                            <x-home.article.section-title :read-more-url="route('premium')">Artikel Premium</x-article.section-title>
                        </div>
                        <div class="splide__track px-4">
                            <div class="splide__list splide__list__grid sm:grid-cols-5 sm:gap-4">
            ...
            <x-home.base.container :paddless="true">
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-5 sm:px-4">
                    <div class="px-4 sm:col-span-full sm:px-0">
                        <x-home.article.section-title :read-more-url="route('category.detail', ['category' => $category->slug])">{{ $category->name }}</x-article.section-title>
                    </div>
                    <div class="px-4 sm:px-0">
                        <x-home.article.card :article="$category->articles->first()" type="editor" />
                    </div>
                        ...
                        <div class="splide__track px-4 sm:px-0">
                            <div class="splide__list splide__list__grid sm:grid-cols-4 sm:gap-4">
                                @foreach ($category->articles->skip(1) as $article)
                                    <x-home.article.card :article="$article" type="category" @class(['splide__slide']) />
                                @endforeach
                            </div>
                        </div>
                    </div>
