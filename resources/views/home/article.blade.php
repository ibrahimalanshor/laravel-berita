...
<x-home.base.container :paddless="true" class="py-4 space-y-4 sm:px-4">
    ...
    <div class="grid grid-cols-1 divide-y divide-neutral-200 gap-6 sm:grid-cols-5 sm:divide-y-0 lg:grid-cols-6">
        <div class="sm:col-span-3 lg:col-span-4 space-y-6 pb-6 sm:pb-0">
                ...
                <header class="space-y-2">
                    ...
                    <div class="text-sm flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-2 sm:flex-col sm:items-end">
                            @auth
                                <x-home.article.action :article="$article" />
                            @endauth
                            <x-home.article.share :article="$article" />
                        </div>
                    </div>
                </header>
                        ...
                        <form class="border border-neutral-300 rounded-md divide-y divide-neutral-300" method="POST" action="{{ route('subscribe.checkout') }}">
                            ...
                            @foreach ($packages as $package)
                                <div class="p-4 flex items-center justify-between">
                                    ...
                                    <x-home.base.button name="period" type="submit" value="{{ $package['period'] }}" :color="$package['featured'] ? 'primary' : 'bordered'" size="custom" class="h-9 px-3 rounded-md text-sm lg:h-10 lg:px-4 lg:text-base">
            ...
            @if (!$article->premium || ($subscription && !$subscription->expired))
                <x-home.article.comment class="px-4 sm:px-0" :article="$article" />
            ...
            <section class="px-4 space-y-4 lg:border-b-0 sm:px-0">
                <x-home.article.section-title>{{ $article->category->name }}</x-article.section-title>

                <div class="grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-6">
                    @foreach ($categoryArticles as $article)
                        <x-home.article.card :article="$article" type="article-category" />
                    @endforeach
                </div>
            </section>
            ...
            <section class="px-4 space-y-4 sm:px-0 md:pb-0">
                <x-home.article.section-title>Artikel Terkait</x-article.section-title>

                <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:gap-6">
                    @foreach ($relatedArticles as $article)
                        <x-home.article.card :article="$article" type="related-article" />
                    @endforeach
                </div>
            </section>
        </div>

        <x-home.article.sidebar class="sm:col-span-2 space-y-6" :bordered-top="false" />
    </div>
</x-base.container>
@endsection
