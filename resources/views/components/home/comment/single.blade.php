        ...
        <div id="report-{{ $comment->id }}" class="hidden shadow-md absolute top-6 right-0 bg-white border border-neutral-200 rounded-md py-1 z-10" data-click-outside-close="dropdown" data-ignore=".open-report-{{ $comment->id }}">
            ...
            <div>
                ...
                <form id="report-{{ $comment->id }}-form" method="POST" action="{{ route('comment.report', ['comment' => $comment->id]) }}" class="space-y-2 hidden px-3 pt-1 pb-2">
                    ...
                    <x-home.base.button color="primary" size="sm" icon="icon-[tabler--message-report-filled]" class="block w-full">Laporkan</x-base.button>
                </form>
            </div>
        </div>
    </div>
    ...
    <div class="grow space-y-2">
        <div>
            <div>
                ...
                @if ($comment->reported)
                    <x-home.base.alert color="light" size="sm" class="italic my-1">
        ...
        @if ($comment->replies->count())
            <div id="comment-list-{{ $comment->id }}" class="space-y-2">
                @foreach ($comment->replies as $reply)
                    <x-home.comment.single :comment="$reply" :reactions="$reactions" />
                @endforeach
            </div>
        ...
        @auth
            @if (!$comment->reply_id)
                <x-home.comment.form type="reply" :reply="$comment" />
            @endif   
        @endauth
    </div>
</article>