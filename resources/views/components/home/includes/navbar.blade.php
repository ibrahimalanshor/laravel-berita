<nav class="bg-neutral-900 h-14 text-white lg:h-15 sticky top-0 z-20">
    <x-home.base.container class="h-full flex items-center justify-between">
        ...
        <div class="flex items-center gap-4">
            ...
            <a href="{{ route('home') }}">
                <x-home.includes.logo class="max-h-6" />
            </a>
        </div>
        ...
        " id="nav-menu" data-click-outside-close="drawer" data-ignore=".open-nav-menu">
            ...
            @guest
                <x-home.base.button icon="icon-[tabler--brand-google-filled]" tag-name="a" color="bordered" class="w-full sm:hidden" href="{{ route('auth.google') }}">
                    <p>Masuk</p>
                </x-base.button>
            @endguest

            @if (!auth()->check())
                <x-home.base.button icon="icon-[tabler--bell-ringing-filled]" tag-name="a" color="primary" class="w-full sm:hidden" href="{{ route('subscribe.index') }}">
                    <p>Berlangganan</p>
                </x-base.button>
            @elseif (!$subscription)
                <x-home.base.button icon="icon-[tabler--star-filled]" tag-name="a" color="primary" class="w-full sm:hidden" href="{{ route('subscribe.index') }}">
                    <p>Upgrade Premium</p>
                </x-base.button>
            @elseif ($subscription->expired)
                <x-home.base.button icon="icon-[tabler--star-filled]" tag-name="a" color="primary" class="w-full sm:hidden" href="{{ route('profile.subscription') }}#perpanjang_langganan">
                    <p>Perpanjang Premium</p>
                </x-base.button>
            @else
                <x-home.base.button icon="icon-[tabler--star-filled]" tag-name="a" color="warning" class="w-full sm:hidden" href="{{ route('profile.subscription') }}">
                    <p>Premium</p>
                </x-base.button>
            @endif
                        ...
                        @else
                            <a href="{{ $url }}" class="flex items-center justify-between lg:hover:text-neutral-400">
                                ...
                                @if ($menu === 'Notifikasi' && $notificationCount > 0)
                                    <x-home.base.badge-pill>{{ $notificationCount }}</x-base.badge-pill>
                                @endif
                            </a>
                        @endif
                    @endforeach
        ...
        <div id="nav-action" class="absolute top-14 bg-neutral-900 left-0 p-4 w-full hidden sm:static sm:p-0 sm:bg-transparent sm:w-auto sm:flex sm:gap-2">
            ...
            @if (!auth()->check())
                <x-home.base.button icon="icon-[tabler--bell-ringing-filled]" tag-name="a" color="primary" class="hidden sm:flex items-center" size="sm" :ignoreDisplay="true" href="{{ route('subscribe.index') }}">
                    <p>Berlangganan</p>
                </x-base.button>
            @elseif (!$subscription)
                <x-home.base.button icon="icon-[tabler--star-filled]" tag-name="a" color="primary" class="hidden sm:flex items-center" size="sm" :ignoreDisplay="true" href="{{ route('subscribe.index') }}">
                    <p>Upgrade Premium</p>
                </x-base.button>
            @elseif ($subscription->expired)
                <x-home.base.button icon="icon-[tabler--star-filled]" tag-name="a" color="primary" class="hidden sm:flex items-center" size="sm" :ignoreDisplay="true" href="{{ route('profile.subscription') }}#perpanjang_langganan">
                    <p>Perpanjang Premium</p>
                </x-base.button>
            @else
                <x-home.base.button icon="icon-[tabler--star-filled]" tag-name="a" color="warning" class="hidden sm:flex items-center" size="sm" :ignoreDisplay="true" href="{{ route('profile.subscription') }}">
                    <p>Premium</p>
                </x-base.button>
            @endif

            @if (!auth()->check())
                <x-home.base.button icon="icon-[tabler--brand-google-filled]" tag-name="a" color="light" class="hidden sm:flex items-center" size="sm" :ignoreDisplay="true" href="{{ route('auth.google') }}">
                    <p>Masuk</p>
                </x-base.button>
            @else
                <div class="hidden sm:flex items-center relative">
                    <button class="open-user-navbar-dropdown flex items-center cursor-pointer relative" aria-label="Buka Profil" data-toggle="dropdown" data-target="#user-navbar-dropdown">
                        ...
                        @if ($notificationCount > 0)
                            <x-home.base.badge-pill class="absolute -top-1 -right-1">
                                {{ $notificationCount }}
                            </x-base.badge-pill>
                        @endif
                    </button>
                        ...
                        @foreach ($userMenus as $menu => $url)
                            ...
                            @else
                                <a href="{{ $url }}" class="block px-2 py-1 whitespace-nowrap flex items-center gap-2 relative hover:bg-neutral-100">
                                    ...
                                    @if ($menu === 'Notifikasi' && $notificationCount > 0)
                                        <x-home.base.badge-pill class="absolute right-2">
                                            {{ $notificationCount }}
                                        </x-base.badge-pill>
                                    @endif
                                </a>
...
@if ($displayTag)
    <div class="border-b border-neutral-200">
        <x-home.base.container :paddless="true">
            <div class="flex gap-2 py-3 overflow-x-auto">
                <div class="w-2 shrink-0"></div>
                @foreach ($tags as $tag)
                    <x-home.tag.link :tag="$tag" />
                @endforeach
                <div class="w-2 shrink-0"></div>
            </div>
        </x-base.container>
