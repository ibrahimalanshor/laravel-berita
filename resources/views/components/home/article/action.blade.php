<div class="relative sm:flex sm:items-center sm:gap-2">
    <button class="text-neutral-700 flex items-center open-action-dropdown sm:hidden" data-toggle="dropdown" data-target="#action-dropdown" aria-label="Bagikan">
        <span class="icon-[tabler--bookmark] size-5"></span>
    </button>
    <p class="hidden md:block text-sm text-neutral-700">Simpan:</p>
    <div
        id="action-dropdown"
        class="hidden absolute bg-white border border-neutral-200 rounded-md shadow-lg right-0 mt-2 py-1 sm:flex sm:static sm:bg-none sm:border-0 sm:shadow-none sm:p-0 sm:m-0 sm:gap-2"
        data-click-outside-close="dropdown"
        data-ignore=".open-action-dropdown"
    >
        <form action="{{ route('article.bookmark', ['article' => $article->slug]) }}" method="POST">
            @csrf
            <button class="flex items-center gap-2 px-3 py-2 whitespace-nowrap sm:p-0 sm:cursor-pointer" type="submit">
                <span class="{{ $bookmarked ? 'icon-[tabler--clock-filled]' : 'icon-[tabler--clock]' }} text-neutral-700 size-4 shrink-0 sm:size-5 {{ $bookmarked ? 'sm:icon-[tabler--bookmark-filled]' : 'sm:icon-[tabler--bookmark]' }}"></span>
                <span class="text-neutral-700 sm:hidden">{{ $bookmarked ? 'Hapus dari Baca Nanti' : 'Baca Nanti' }}</span>
            </button>
        </form>
        <form action="{{ route('article.favorite', ['article' => $article->slug]) }}" method="POST">
            @csrf
            <button class="flex items-center gap-2 px-3 py-2 whitespace-nowrap sm:p-0 sm:cursor-pointer" type="submit">
                <span class="{{ $addedToFavorite ? 'icon-[tabler--heart-filled] text-red-700' : 'icon-[tabler--heart] text-neutral-700' }} size-4 shrink-0 sm:size-5 {{ $addedToFavorite ? 'sm:icon-[tabler--heart-filled]' : 'sm:icon-[tabler--heart]' }}"></span>
                <span class="text-neutral-700 sm:hidden">{{ $addedToFavorite ? 'Hapus dari Favorit' : 'Tambah Ke Favorit' }}</span>
            </button>
        </form>
    </div>
</div>